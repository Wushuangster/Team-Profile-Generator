...
const employeeQuestions = [
    ...
    {
        type:'input',
        message: 'Waht is the employee\'s employee ID',
        name: 'id',
    },
    {
        type: 'input',
...
function writeToFile(fileName, data, header, footer) {
     ...
     err ? console.error(err) : console.log('HTML Innitially Stored!')
     );
...
function appendToFile(fileName,data,header, footer) {
    fs.appendFile(fileName, generateHTML(data,header, footer),err =>
       err ? console.error(err): console.log('HTME Appended!')
    );
}
...
function generateCard(id,name,role,email, trait) {
    ...
    `\n\t\t\t<div class=\"col-3 border p-2 m-2 secondary flex-column\">
               <div class=\"card\">
                  <div class=\"card-body test-white bg-primary\">
                     ...
                     <ul class=\"list-group list-group-flush bg-secondary\">
                         ...
                         <li class=\"list-group-item\">
                             ${role == 'Manager' ? ('Office: '+ trait)
                              :role == 'Engineer' ? 'Github: <a href=\"https://github.com/${trait}\" target=\"_blank\" className=\"card-link\">${trait}</a>'
                              :'School: ' + trait}
                         </li>
                     </ul>
                 </div>
